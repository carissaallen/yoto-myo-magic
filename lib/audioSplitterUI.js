...
if (typeof showImportModal === 'undefined') {
  window.showImportModal = function(audioFiles, trackIcons, coverImage, defaultName, sourceType) {
    window.postMessage({
      type: 'YOTO_SHOW_IMPORT_MODAL',
...
function getAudioSplitterURL() {
  const metaTag = document.querySelector('meta[name="audio-splitter-url"]');
  return metaTag ? metaTag.content : null;
}

window.showAudioSplitterModal = function() {
  if (!window.AudioSplitter) {
    const audioSplitterURL = getAudioSplitterURL();
    if (!audioSplitterURL) {
      console.error('Audio Splitter URL not found in meta tag');
      return;
    }

    const script = document.createElement('script');
    script.src = audioSplitterURL;
    script.onload = () => {
      createAudioSplitterModal();
    };
    script.onerror = () => {
      console.error('Failed to load audio splitter script');
...
function createAudioSplitterModal() {
  ...
  content.innerHTML = `
    ...
    <div id="audio-chapters-section" style="display: none;">
      ...
      <div style="padding: 8px 12px; background: #f9fafb; border: 1px solid #e5e7eb; border-radius: 8px 8px 0 0; display: flex; align-items: center; gap: 12px;">
        <span style="color: #6b7280; font-size: 12px; font-weight: 600; min-width: 30px;">#</span>
        <span style="color: #6b7280; font-size: 12px; font-weight: 600; flex: 1;">Chapter Name</span>
        <span style="color: #6b7280; font-size: 12px; font-weight: 600;">MM:SS</span>
        <span style="color: #6b7280; font-size: 12px; font-weight: 600; width: 60px; text-align: center;">Preview</span>
        <span style="color: #6b7280; font-size: 12px; font-weight: 600; width: 60px; text-align: center;">Merge</span>
      </div>
      <div id="chapters-list" style="max-height: 300px; overflow-y: auto; border: 1px solid #e5e7eb; border-top: none; border-radius: 0 0 8px 8px; padding: 12px; padding-top: 0;">
      </div>

      <div style="margin-top: 20px;">
        <label for="split-playlist-name" style="display: block; margin-bottom: 8px; color: #374151; font-weight: 500;">
          Playlist Name:
        </label>
...
async function handleAudioFileUpload(event) {
  ...
  // Limit to 100MB to prevent browser crashes
  if (file.size > 100 * 1024 * 1024) {
    const sizeMB = (file.size / (1024 * 1024)).toFixed(1);
    console.error(`File too large: ${sizeMB}MB (limit is 100MB)`);
    showNotification(`File too large: ${sizeMB}MB. Please select a file under 100MB to prevent browser crashes.`, 'error');
  ...
  try {
    ...
    if (!window.AudioSplitter) {
      console.error('AudioSplitter not loaded. Loading it now...');
      const audioSplitterURL = getAudioSplitterURL();
      if (audioSplitterURL) {
...
// Make handleAudioFileUpload available globally for debugging
...
function displayDetectedChapters(chapters, originalFile, splitter) {
  ...
  chapters.forEach((chapter, index) => {
    ...
    chapterItem.innerHTML = `
      ...
      ${index > 0 ? `
      <button class="merge-chapter-btn" data-chapter-index="${index}" style="
        background: transparent;
        border: 1px solid #d1d5db;
        border-radius: 4px;
        padding: 4px 8px;
  ...
  document.querySelectorAll('.merge-chapter-btn').forEach(btn => {
    btn.addEventListener('click', (e) => {
      const index = parseInt(e.currentTarget.dataset.chapterIndex);
      mergeChapterWithPrevious(index, originalFile, splitter);
    });
  });
...
async function mergeChapterWithPrevious(chapterIndex, originalFile, splitter) {
  const modal = document.querySelector('#yoto-audio-splitter-modal');
  if (!modal || !modal.audioChapters || chapterIndex === 0) return;
  ...
  const totalLength = previousChapter.audioBuffer.length + currentChapter.audioBuffer.length;
  const mergedBuffer = audioContext.createBuffer(
    previousChapter.audioBuffer.numberOfChannels,
    totalLength,
    previousChapter.audioBuffer.sampleRate
  );

  // Copy both buffers into the merged buffer
  for (let channel = 0; channel < mergedBuffer.numberOfChannels; channel++) {
    const outputData = mergedBuffer.getChannelData(channel);
    const prevData = previousChapter.audioBuffer.getChannelData(channel);
    const currData = currentChapter.audioBuffer.getChannelData(channel);

    outputData.set(prevData, 0);
    outputData.set(currData, previousChapter.audioBuffer.length);
  }

  // Update previous chapter with merged data
  previousChapter.audioBuffer = mergedBuffer;
  previousChapter.duration = previousChapter.duration + currentChapter.duration;
  previousChapter.name = `${previousChapter.name} (merged)`;
  ...
  displayDetectedChapters(chapters, originalFile, splitter);
}
